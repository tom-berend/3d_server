...
// userid '00000000' and '00000001' are test users, not real
class Users extends AbstractData implements BasicTableFunctions
{
   ...
   // Hold a singleton instance of the class
    ...
    {    // private, so can't instantiate from outside class
        ...
        $this->uuidPrefix   = 'S';
        $this->primaryKey   = 'uuid';
        $this->secondaryKeys= array('studentid');
        $this->createString =
            "CREATE TABLE IF NOT EXISTS `{$this->tableName}` (
              `uuid`      	            varchar(16)  NOT NULL,
              `userid`                  varchar(32)  NOT NULL,
              `block`                   int(1) unsigned default 0,
              `role` 	                varchar(32),
              `parentid1`               varchar(32),
              `parentid2`               varchar(32),
              `parentid3`               varchar(32),
              `cargo` 	                text,
              `created`                 int(10) unsigned default 0,
              `createdhuman`            varchar(32),
              `lastupdate`              int(10) unsigned default 0,
              `lastbackup`              int(10) unsigned default 0,
              `nextnag`                 int(10) unsigned default 0,
    ...
    public function updateUser($userID,$cargo){
        // step 1, make sure the user record exists
        $stmt = "SELECT userID from  {$this->tableName} where userid = ".$this->quote_string($userID);
        $ret = $session->query($stmt);
        if(count($ret)==0){
            // no record, so we'll insert one
            $aArray = array("userID"=>"studentID", "cargo"=>$cargo);
            $test->insertArray($aArray);  // $aArray is a set of field-value pairs
        } else {
            // simple update
            $aArray = array("cargo"=>$cargo);
            $where = "userID = ".$this->quote_string($userID);
            $test-> updateArray($aArray, $where);
        }
    }
...
{    // don't extend UnitTestCase because Factory does
    ...
    public function __construct()
    {
        ...
        if (!self::$dbc) {   // only need to create a connection once
            ...
            self::$dbc = new mysqli($GLOBALS['host'], $GLOBALS['user'], $GLOBALS['pswd'], $GLOBALS['db']);
